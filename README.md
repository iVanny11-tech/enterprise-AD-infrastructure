...
AWS (EC2, VPC, Subnets, Security Groups, Elastic IP, Systems Manager, IAM)
...
PowerShell (New-ADUser, Get-ADUser, Set-Acl, Unlock-ADAccount, etc.)
...
Remote Desktop Protocol (RDP), AWS Systems Manager Session Manager
...
nslookup, ping, Event Viewer, Windows Firewall logs
...
AWS account configured with an IAM admin user (least-privilege principle applied where practical for a lab context)



Custom VPC (10.0.0.0/16) with a public subnet (10.0.1.0/24), internet gateway, and route table



Security group (Enterprise-Server-SG) initially scoped to RDP/SSH/HTTP/HTTPS, later expanded to permit internal VPC traffic (see Incident 3)
...
Key pair (Enterprise-Lab-Key) used to decrypt initial local Administrator passwords for both instances
...
Unlocked jsmith's account after lockout
...
Provisioned Priya Nair into the Sales department (OU, groups, drive mapping)
...
Disabled mtorres's account per departure process
...
To demonstrate the client side of the domain — logging in as a standard domain user, verifying group-driven access, and confirming drive mapping — CLIENT01 was provisioned and joined to enterprise.local. This phase is where Incident 3 occurred; see Challenges & Troubleshooting for the full breakdown.
...
9 (including Priya Nair; excludes disabled Mike Torres)
...
Incidents diagnosed & resolved/documented
...
AWS Documentation — VPC Security Groups
...
LinkedIn · GitHub · yivan56@gmail.com
